...
@section('content')
    ...
    <div class="col-md-8 offset-md-2">
        <div class="card-box">
            ...
            <h4 class="m-t-0 m-b-30 text-center addFormHeader header-title">Update Membership</h4>

            <form class="form-horizontal myForms" action="/profile/membership/update/{{$membership->membershipid}}" method="POST" role="form">
                @csrf
                    ...
                    <div class="col-md-9">
                        <select name="userid" class="form-control">
                            ...
                            <option value="{{Auth::id()}}"
                                    {{ (old('userid') ?? $membership->userid) == Auth::id() ? 'selected' : ''}} disabled readonly="">
                                {{ Auth::user()->getFullname() }}
                            </option>

                            @foreach(Auth::user()->getChildren() as $child)
                                <option value="{{$child->userid}}"
                                        {{ (old('userid') ?? $membership->userid) == $child->userid ? 'selected' : ''}}  disabled readonly="">
                                    {{ $child->getFullName() }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="organisationid" class="form-control">
                            ...
                            @foreach($organisations as $organisation)
                                <option value="{{$organisation->organisationid}}"
                                        {{ (old('organisationid') ?? $membership->organisationid) == $organisation->organisationid ? 'selected' : ''}}>
                                    {{ $organisation->label }}
                                </option>
                            @endforeach
                        </select>
